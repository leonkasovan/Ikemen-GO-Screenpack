...
void main() {
    ...
    float t = iTime * 2.5* p0;
    
    vec2 distortion = vec2(0.0);
    
    distortion.x += sin(uv.y * 14.0 + t * 1.8) * 0.045;
    distortion.x += sin(uv.y * 7.0 + t * 3.3) * 0.022;
    
    distortion.y += sin(uv.x * 19.0 + t * 2.1) * 0.038;
    distortion.y += cos(uv.x * 11.0 + t * 4.7) * 0.019;
    
    distortion.x += sin((uv.x + uv.y) * 23.0 + t * 2.9) * 0.012;
    distortion.y += cos((uv.x - uv.y) * 17.0 + t * 1.4) * 0.015;
    
    float edgeBoost = length(uv - 0.5) * 0.6 + 0.4;
    distortion *= edgeBoost;
    
    vec2 sampleUV = uv + distortion;
    
    vec4 baseColor = GetIkemenPixel(sampleUV);
